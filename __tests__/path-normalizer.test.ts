...
describe('PathNormalizer', () => {
  ...
  describe('constructor options', () => {
    ...
    it('should respect custom delimiter', () => {
      ...
      instance.addRule(['user', 'profile'], (matched) => matched.join('/'));
    ...
    it('should handle case-insensitive matching when enabled', () => {
      ...
      instance.addRule(['USER', 'EMAIL'], () => 'profile.email');
    ...
    it('should throw on unmatched paths when throwOnUnmatched is true', () => {
      ...
      expect(() => instance.normalizePath('unknown.path')).toThrow(
        'No matching rule for path: unknown.path'
      );
    });
  });
  ...
  describe('exact string matching', () => {
    it('should match exact path segments', () => {
      normalizer.addRule(['user', 'email'], () => 'profile.contact.email');
    ...
    it('should not match partial paths', () => {
      normalizer.addRule(['user', 'email', 'primary'], () => 'profile.email');
    ...
    it('should handle paths with bracket notation', () => {
      ...
      normalizer.addRule(['user', '0', 'email'], () => 'users.0.emailAddress');
  ...
  describe('wildcard matching', () => {
    ...
    it('should capture wildcard values', () => {
      normalizer.addRule(['*', 'settings', '*'], (_, context) => {
    ...
    it('should match deep wildcard (**)', () => {
      normalizer.addRule(['api', '**'], (_, context) => {
    ...
    it('should handle deep wildcard at the end of pattern', () => {
      normalizer.addRule(['config', 'locales', '**'], (_, context) => {
    ...
    it('should match empty path with deep wildcard', () => {
      normalizer.addRule(['data', '**'], (_, context) => {
        const remaining = context.segments.slice(1);
  ...
  describe('regex matching', () => {
    it('should match paths with regex patterns', () => {
      normalizer.addRule([/^user_\d+$/, 'email'], (matched) => `users.${matched[0]}.emailAddress`);
    ...
    it('should not match when regex does not match', () => {
      normalizer.addRule([/^user_\d+$/, 'email'], () => 'matched');
  ...
  describe('custom matcher functions', () => {
    ...
    it('should provide all context to custom matcher', () => {
      ...
      normalizer.addRule(['special', matchSecondSegment], (matched) => `validated.${matched[1]}`);
    ...
    it('should handle complex validation in custom matcher', () => {
      ...
      normalizer.addRule(['i18n', isValidLocale, 'messages'], (_, context) => {
        const locale = context.segments[1];
        const index = validLocales.indexOf(locale ?? '');
        return `translations[${index}].content`;
      });

      expect(normalizer.normalizePath('i18n.en.messages').normalized).toBe(
        'translations[0].content'
      );
      expect(normalizer.normalizePath('i18n.es.messages').normalized).toBe(
        'translations[3].content'
      );
  ...
  describe('transform functions', () => {
    it('should transform matched paths', () => {
      normalizer.addRule(['old', 'path'], () => 'new.location');
    ...
    it('should return array from transform function', () => {
      normalizer.addRule(['user', 'settings'], () => ['profile', 'preferences', 'general']);
    ...
    it('should handle null return (path removal)', () => {
      normalizer.addRule(['deprecated', '*'], () => null);
    ...
    it('should have access to full context in transform', () => {
      normalizer.addRule(['data', '*', '*'], (matched, context) => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle paths with quoted bracket notation', () => {
      ...
      normalizer.addRule(['user', 'email', 'primary'], () => 'user.email.main');
  ...
  describe('complex real-world scenarios', () => {
    it('should handle form field localization', () => {
      ...
      normalizer.addRule(['fields', (s) => locales.includes(s), '*'], (_, context) => {
        const locale = context.segments[1];
      ...
      expect(normalizer.normalizePath('fields.en.title').normalized).toBe(
        'localization[0].fields.title'
      );
      expect(normalizer.normalizePath('fields.de.description').normalized).toBe(
        'localization[2].fields.description'
      );
    });

    it('should handle nested array indices', () => {
      normalizer.addRule(['items', /^\d+$/, 'subitems', /^\d+$/, 'value'], (_, context) => {
